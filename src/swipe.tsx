...
export const Carousel: FC<IProps> = props => {
    ...
    const contentH = listItemRef.current.reduce((sum, cur) => sum + cur.height, 0);

    const whRef = useRef<String>()
    const swiper = (wh: string) => {
        if (!whRef.current) {
            whRef.current = wh;
        }
        let init = true;
    ...
    }, [contentLayout.height, containerLayout.height, iterations, length]);
        ...
        >
            ...
            >
                {list.map((item, index) => <View
                    style={[styles.item]}
                    onLayout={event => {
                        const layout = event?.nativeEvent?.layout || {} as LayoutRectangle
